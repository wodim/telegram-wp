...
namespace Telegram.EmojiPanel
{
    ...
    public static class BrowserNavigationService
    {
        ...
        private static void OnTextChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            var message = GetMessage(textBox) as TLMessageBase;
            ...
            var splitData = suppressParsing ? new List<string> { newText }: ParseText(newText);
            ...
            foreach (var splStr in splitData)
            {
                ...
                {
                    AddRawText(textBox, par, innerSplit[0]);
                }
                else if (innerSplit.Length > 1)
                {
                    ...
                    {
                        var hyp = GetHyperlink(innerSplit[1], innerSplit[0], (h, navstr) => NavigateOnHyperlink(navstr, message), textBox.Foreground);
                        par.Inlines.Add(hyp);
                    }
                }
            }
        ...
        private static void AddRawText(RichTextBox text_block, Paragraph par, string raw_text)
        {
            ...
            while (cont)
            {
                ...
                if (Emoji.Dict.ContainsKey(bytesStr))
                {
                    ...
                    double imageHeight;
                    par.Inlines.Add(GetImage(bytesStr, out imageHeight));
        ...
        private static void NavigateOnHyperlink(string navstr, TLMessageBase message)
        {
            ...
            else if (navstr.StartsWith("tlg://?action=command"))
            {
                ...
                if (commandIndex != -1)
                {
                    ...
                    RaiseInvokeCommand(new TelegramCommandEventArgs { Command = command, Message = message });
                }
            }
        ...
        public static List<string> ParseText(string html)
        {
            ...
            html = rx.Replace(html, delegate(Match m)
            {
                ...
                //if (full.IndexOf('/') == 0 && IsValidCommand(full))
                //    return string.Format("\atlg://?action=command&q={0}\b{1}\a", full, full);
                ...
                .ReplaceByRegex("(^|[\\s])\\/[A-Za-z_0-9]{1,64}(@[A-Za-z_0-9]{5,32})?([\\w]|$)", " \atlg://?action=command&q=$0\b$0\a");
            html = html.Replace("\n ", "\n").Replace(" \n", "\n");
