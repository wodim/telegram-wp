...
namespace TelegramClient.Utils
{
    ...
    public static class Language
    {
        ...
        /// <returns></returns>   
        public static string RuDeclension(int number, string nominative, string genitiveSingular, string genitivePlural)
        {
            ...
            {
                return string.Format("{0} {1}", number, genitiveSingular);
            }

            return string.Format("{0} {1}", number, genitivePlural);
        }
        ...
        /// <returns></returns>   
        public static string EnDeclension(int number, string singular, string plural)
        {
            return string.Format("{0} {1}", number, number == 1 || number == 0 ? singular : plural);
        }

        public static string Declension(int number, string nominativeSingular, string nominativePlural, string genitiveSingular, string genitivePlural, string enException = null)
        {
            ...
            //{
            //    return RuDeclension(number, nominativeSingular, genitiveSingular, genitivePlural);
            //}

            if (!string.IsNullOrEmpty(enException)) return enException;

            return EnDeclension(number, nominativeSingular, nominativePlural);
        }
        ...
        public static void Test()
        {
            for (var i = 0; i < 105; i++)
            {
                Debug.WriteLine(RuDeclension(i, "собеседник", "собеседника", "собеседников"));
            }
