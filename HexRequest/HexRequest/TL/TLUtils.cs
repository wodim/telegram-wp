...
namespace Telegram.Api.TL
{
    public static partial class TLUtils
    {
        ...
        public static bool IsValidAction(TLObject obj)
        {
            ...
            var sendEncryptedAction = obj as TLSendEncrypted;
            if (sendEncryptedAction != null)
            {
                return true;
            }

            var sendEncryptedFileAction = obj as TLSendEncryptedFile;
            if (sendEncryptedFileAction != null)
            {
                return true;
            }

            var sendEncryptedServiceAction = obj as TLSendEncryptedService;
            if (sendEncryptedServiceAction != null)
            {
                return true;
            }

            var readEncryptedHistoryAction = obj as TLReadEncryptedHistory;
            ...
            TLInt replyToMsgId)
        {
            ...
            if (m.FromId != null) m.SetFromId();
            if (m._media != null) m.SetMedia();
            if (m.ReplyToMsgId != null) m.SetReply();
#else
            var m = new TLMessage36
            {
                FromId = fromId,
